...
export function createGame(dom){
  class Game{
    ...
    setCamMode(mode){
      ...
      dom.mode.textContent='v31 • '+(mode==='third'?'THIRD PERSON':'FIRST PERSON')+' • LMB/RMB + DRAG • V = '+(mode==='third'?'FIRST PERSON':'THIRD PERSON');
    }
  ...
  try{
    ...
    dom.flashNotice('نسخه ۳۱ — کلانتری + بار/سالون با چیدمان واقعی');
  }catch(err){
