...
export function createGame(dom){
  class Game{
    ...
    setCamMode(mode){
      ...
      dom.mode.textContent='v32 • '+(mode==='third'?'THIRD PERSON':'FIRST PERSON')+' • LMB/RMB + DRAG • V = '+(mode==='third'?'FIRST PERSON':'THIRD PERSON');
    }
  ...
  try{
    ...
    dom.flashNotice('نسخه ۳۲ — کلانتری + بار/سالون با چیدمان واقعی');
  }catch(err){
